...
const Chats = ({setTabs}) => {
  ...
  const handleClick = async (contact) => {
    ...
    const newContact  = await postNewContact({userId: userData.id,contactId:contact.userId})

    if(newContact?.error){
      return toast.error(newContact.error)
    }

    dispatch(setSelectedUser(newContact));
    setTabs(1)
  }
